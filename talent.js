  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        // 人格担保：在高二时如果发生未达到晋级线时自动发动，高一默认不发动，IOI选拔不能发动
        this.registerTalent({
          name: '人格担保',
          description: '高二时在NOI系列比赛中未晋级时自动发动天赋强行晋级，但之后所有比赛中所有属性降低至70%。',
          color: '#4A90E2',
          prob: 0.03, // 可以随机获取
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              // 监听比赛结束预检查事件（在分数线计算之后，正式contest_finish事件之前）
              if(eventName !== 'contest_finish_precheck') return null;
              if(!ctx || !ctx.state) return null;
              ...
              // 只在NOI系列比赛且未晋级时触发，且不能是IOI选拔
              // 修正逻辑：只有当是NOI系列比赛、不是IOI选拔、且未晋级时才触发
              if(!isNoiSeries) return null;
              if(isIOISelection) return null;
              ...
              // 判断是否在高二（第二学年）
              // 假设每学年14周，第二学年从第15周开始
              const SEASON_WEEKS = (typeof window !== 'undefined' && typeof window.SEASON_WEEKS !== 'undefined') ? window.SEASON_WEEKS : 14;
              const isSecondYear = game.week > SEASON_WEEKS;
              
              console.log(`[人格担保调试] 学生: ${student.name}, 比赛: ${contestName}, 周数: ${game.week}, 赛季周数: ${SEASON_WEEKS}, 是否高二: ${isSecondYear}, 是否晋级: ${passed}`);

              // 只有在高二时才自动发动
              if (isSecondYear) {
                // 初始化 talent_state 并设置可以使用人格担保
                student._talent_state = student._talent_state || {};
                student._talent_state._can_use_personal_guarantee = true;
                
                // 自动发动人格担保
                if (window.TalentManager.activatePersonalGuarantee(student)) {
                  console.log(`[人格担保调试] ${student.name} 自动发动人格担保成功`);
                  return { 
                    action: 'personal_guarantee_auto_activated', 
                    message: `人格担保自动发动：${student.name} 在${contestName}中未晋级，已自动使用人格担保强行晋级，但之后所有比赛中属性将降低至70%！` 
                  };
                } else {
                  console.log(`[人格担保调试] ${student.name} 自动发动人格担保失败`);
                }
              } else {
                console.log(`[人格担保调试] ${student.name} 不在高二，不发动人格担保`);
              }
              // 高一时不发动，也不提供选择
              return null;
